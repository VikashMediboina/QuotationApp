...
pool.query("SELECT line_item_id from line_item ORDER BY line_item_id DESC LIMIT 1").then((results) => {
    ...
    pool.query(`Insert into line_item ("line_item_id", "line_item_title","line_item_desc","room_type","unit_price","inserted_by","inserted_date")  
...
const get_all_line_items=()=> new Promise((resolve,reject)=>{
    ...
    const update_line_item_service = (body) => new Promise((resolve, reject) => {
        ...
        return pool.query(`UPDATE line_item SET "line_item_title"=$1 , "line_item_desc"=$2,"room_type"=$3,"unit_price"=$4,"updated_by"=$5,"updated_date"=$6 WHERE "line_item_id" = $7`,
        [ body.line_item_title, body.line_item_desc,body.room_type,body.unit_price, body.updated_by, body.updated_date,body.line_item_id],
        )
        .then((results) => {
            console.log(results)
